...
Int_t rebin[nPtBins+1]=    {4,10,10,10,8,6,12,10,10,20,24,30,38};
Int_t rebinRflAn=1;
...
Int_t factor4refl=0;
Float_t massRangeForCounting[nPtBins]={0.03,0.03,0.03,0.03,0.035,0.05,0.05,0.055,0.055,0.06,0.08,0.08}; // GeV 0-36

const Int_t nsamples=1;

Bool_t LoadD0toKpiMCHistos(TString fileName, TH1F** hSig, TH1F** hBkg, TH1F** hRfl, const char *CutsType);
Bool_t LoadD0toKpiHistos(TObjArray* listFiles, TH1F** hMass, const char *CutsType);

void Reflections(TString fileNameMC="../../../../../../mctreni/AnalysisResults440.root", TString fileNameb="AnalysisResults1406_b.root", TString fileNamec="AnalysisResults1407_c.root", TString fileNamed="AnalysisResults1408_d.root", TString fileNamee="AnalysisResults1409_e.root", const char *CutsTypeMC="_massD0_pass4_central3", const char *CutsType="_pass4_central_kFirst3"){
...
Bool_t LoadD0toKpiMCHistos(TString fileName, TH1F** hSig, TH1F** hBkg, TH1F** hRfl, const char *CutsType){
  ...
  listmassname += "0100";
  printf("List name %s\n",listmassname.Data());
  TList *hlist=(TList*)dir->Get(listmassname);
  ...
  AliRDHFCutsD0toKpi *dcuts=(AliRDHFCutsD0toKpi*)dir->Get(cutsobjname);
  if(!dcuts) {
  ...
  for(Int_t i=0;i<nPtBinsCuts;i++){
    ...
    if(ptlimsCuts[i]>=ptlims[iFinBin] && 
      ...
      htempS=(TH1F*)hlist->FindObject(Form("histSgn_%d",i));
	...
	hSig[iFinBin]->Add(htempS);
      ...
      htempB=(TH1F*)hlist->FindObject(Form("histBkg_%d",i));
	...
	hBkg[iFinBin]->Add(htempB);
      ...
      htempR=(TH1F*)hlist->FindObject(Form("histRfl_%d",i));
