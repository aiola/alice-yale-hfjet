...
def PlotUncertainties(results, name):
    ...
    paveALICE.AddText("Charged Jets, Anti-#it{k}_{T}, #it{R} = 0.4, |#eta_{jet}| < 0.5, 5 < #it{p}_{T,ch jet} < 15 GeV/#it{c}")
...
def main():
    ...
    f = open("JetZSpectrum_JetPt_5_15_Systematics.yaml", 'r')
    config = yaml.load(f)
    f.close()
    ...
    canvas.SaveAs("{0}/Uncertainties_JetZSpectrum_JetPt_5_15_Paper.pdf".format(config["input_path"]))
    canvas.SaveAs("{0}/Uncertainties_JetZSpectrum_JetPt_5_15_Paper.C".format(config["input_path"]))
