...
class DMesonJetAnalysisEngine:
    ...
    def PlotSpectrum1D(self, s):
        ...
        if s.fLikeSignTotalHistogram:
            self.PlotLikeSignBackground(s)

        if s.fSideBandWindowTotalHistogram:
            self.PlotSideBandBackground(s)

    def PlotLikeSignBackground(self, s):
        c = ROOT.TCanvas("{0}_canvas".format(s.fHistogram.GetName()), s.fHistogram.GetName(), 700, 700)
        self.fCanvases.append(c)
        c.Divide(1, 2)
        padMain = c.cd(1)
        ...
        hSig = s.fUnlikeSignTotalHistogram.DrawCopy()
        hSig.SetMarkerColor(ROOT.kBlue+2)
        hSig.SetMarkerStyle(ROOT.kFullCircle)
        hSig.SetMarkerSize(0.9)
        hSig.SetLineColor(ROOT.kBlue+2)
        hSig.GetYaxis().SetTitleFont(43)
        hSig.GetYaxis().SetTitleSize(26)
        hSig.GetYaxis().SetLabelFont(43)
        hSig.GetYaxis().SetLabelSize(22)
        hSig.GetYaxis().SetTitleOffset(1.4)
        
        hBkg = s.fLikeSignTotalHistogram.DrawCopy("same")
        hBkg.SetMarkerColor(ROOT.kRed+2)
        hBkg.SetMarkerStyle(ROOT.kOpenCircle)
        hBkg.SetMarkerSize(1.0)
        hBkg.SetLineColor(ROOT.kRed+2)
        if hBkg.GetMinimum() < hSig.GetMinimum():
            hSig.SetMinimum(hBkg.GetMinimum() * 0.8)
        if hBkg.GetMaximum() > hSig.GetMaximum():
            hSig.SetMaximum(hBkg.GetMaximum() * 1.2)
        
        leg = ROOT.TLegend(0.54, 0.67, 0.99, 0.82, "", "NB NDC")
        leg.SetBorderSize(0)
        leg.SetFillStyle(0)
        leg.SetTextFont(43)
        leg.SetTextSize(25)
        leg.SetMargin(0.2)
        leg.AddEntry(hSig, "Unlike Sign", "pe")
        leg.AddEntry(hBkg, "Like Sign", "pe")
        leg.Draw()
        ...
        hRatio = s.fUnlikeSignTotalHistogram.DrawCopy()
        hRatio.Divide(s.fLikeSignTotalHistogram)
        hRatio.SetMarkerColor(ROOT.kBlue+2)
        hRatio.SetMarkerStyle(ROOT.kFullCircle)
        hRatio.SetMarkerSize(0.9)
        hRatio.SetLineColor(ROOT.kBlue+2)
        ...
        hRatio.GetYaxis().SetTitle("ratio US / LS")
    ...
    def PlotSideBandBackground(self, s):
        c = ROOT.TCanvas("{0}_canvas".format(s.fHistogram.GetName()), s.fHistogram.GetName(), 700, 700)
        self.fCanvases.append(c)
        c.Divide(1, 2)
        padMain = c.cd(1)
        ...
        hSig = s.fSignalWindowTotalHistogram.DrawCopy()
        hSig.SetMarkerColor(ROOT.kBlue+2)
        hSig.SetMarkerStyle(ROOT.kFullCircle)
        hSig.SetMarkerSize(0.9)
        hSig.SetLineColor(ROOT.kBlue+2)
        ...
        hBkg = s.fSideBandWindowTotalHistogram.DrawCopy("same")
        hBkg.SetMarkerColor(ROOT.kRed+2)
        hBkg.SetMarkerStyle(ROOT.kOpenCircle)
        hBkg.SetMarkerSize(1.0)
        hBkg.SetLineColor(ROOT.kRed+2)
        ...
        leg.SetTextSize(25)
        leg.SetMargin(0.2)
        leg.AddEntry(hSig, "Signal Window", "pe")
        leg.AddEntry(hBkg, "Side Band Window", "pe")
        leg.Draw()
        ...
        hRatio = s.fSignalWindowTotalHistogram.DrawCopy()
        hRatio.Divide(s.fSideBandWindowTotalHistogram)
        hRatio.GetXaxis().SetTitleFont(43)
        hRatio.GetXaxis().SetTitleSize(26)
        hRatio.GetXaxis().SetLabelFont(43)
        hRatio.GetXaxis().SetLabelSize(22)
        ...
        hRatio.GetYaxis().SetTitleOffset(1.4)
        hRatio.GetXaxis().SetTitleOffset(2.9)
        hRatio.GetYaxis().SetTitle("ratio Signal / SB")
        hRatio.GetYaxis().SetNdivisions(504)
