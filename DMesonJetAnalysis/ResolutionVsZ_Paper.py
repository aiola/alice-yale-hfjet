...
def ResolutionComparison(config):
    ...
    h.GetXaxis().SetTitle("(#it{z}_{||,D}^{det} - #it{z}_{||,D}^{part}) / #it{z}_{||,D}^{part}")
    ...
    paveALICE.AddText("PYTHIA6 Perugia-2011, pp, #sqrt{#it{s}} = 7 TeV")
    ...
    leg.SetTextSize(19)
    leg.SetMargin(0.15)
    entry = leg.AddEntry(None, "15 < #it{p}_{T,ch jet}^{part} < 30 GeV/#it{c}", "")
