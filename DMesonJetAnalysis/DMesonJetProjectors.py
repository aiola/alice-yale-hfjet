...
def GetWeightObject(conf, trigger, DMesonDef, jetName):
    if conf:
        doNotInvert = False
        ...
        if "file_name" in conf:
            ...
            effWeight = EfficiencyWeightCalculator.fromFileName(eff_file_name, eff_list_name, eff_obj_name, doNotInvert)
        elif "function_name" in conf:
            effWeight = EfficiencyWeightCalculator.fromFunctionName(conf["function_name"], conf["min"], conf["max"], doNotInvert)
        else:
...
class EfficiencyWeightCalculator:
    ...
    def __init__(self, obj, doNotInvert=False):
        if not obj:
            print("EfficiencyWeightCalculator.__init__: No object provided!!")
            exit(1)
