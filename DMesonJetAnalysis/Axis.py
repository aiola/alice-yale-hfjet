...
class Axis:
    ...
    def GetVariableName(self, label=""):
        ...
        elif self.fName == "jet_bkgpt":
            if label:
                title = "#it{{p}}_{{T,{jetLabel}}}^{{{label}}} - #it{{p}}_{{T,{jetLabel}}}^{{sub,{label}}}".format(jetLabel=jetLabel, label=label)
            else:
                title = "#it{{p}}_{{T,{jetLabel}}} - #it{{p}}_{{T,{jetLabel}}}^{{sub}}".format(jetLabel=jetLabel)
