...
def PlotUncertainties(results, name):
    ...
    tot_unc_low.GetXaxis().SetRangeUser(0.4, 1.0)
    ...
    stat_unc.GetXaxis().SetRangeUser(0.4, 1.0)
    ...
    tot_rel_syst_unc_up.GetXaxis().SetRangeUser(0.4, 1.0)
    ...
    tot_rel_syst_unc_low.GetXaxis().SetRangeUser(0.4, 1.0)
    ...
    for hUp, hLow in zip(results["Uncertainties"]["PartialSystematicUncertaintiesUp"], results["Uncertainties"]["PartialSystematicUncertaintiesLow"]):
        ...
        h_copy.GetXaxis().SetRangeUser(0.4, 1.0)
    ...
    paveALICE = ROOT.TPaveText(0.135, 0.77, 0.53, 0.93, "NB NDC")
    ...
    paveALICE.AddText("Charged Jets, Anti-#it{k}_{T}, #it{R} = 0.4, |#eta_{jet}| < 0.5, 15 < #it{p}_{T,ch jet} < 30 GeV/#it{c}")
...
def main():
    ...
    f = open("JetZSpectrum_JetPt_15_30_Systematics.yaml", 'r')
    config = yaml.load(f)
    f.close()
    ...
    canvas.SaveAs("{0}/Uncertainties_JetZSpectrum_JetPt_15_30_Paper.pdf".format(config["input_path"]))
    canvas.SaveAs("{0}/Uncertainties_JetZSpectrum_JetPt_15_30_Paper.C".format(config["input_path"]))
