...
def PlotInvMass(invMassPlotList, invMassPlotListLS, spectrumPlotListSB, spectrumPlotListLS, config):
    ...
    for i,(bin,binTitle) in enumerate(zip(bins, binTitles)):
        ...
        invMassHistoLS_copy = invMassHistoLS.DrawCopy("same")
        #invMassHistoLS_copy.SetFillStyle(3001)
        #invMassHistoLS_copy.SetFillColor(ROOT.kGreen+2)
        invMassHistoLS_copy.SetLineColor(ROOT.kGreen+2)
        invMassHistoLS_copy.SetLineStyle(1)
        invMassHistoLS_copy.SetLineWidth(1)
        invMassHistoLS_copy.SetMarkerSize(0.9)
        invMassHistoLS_copy.SetMarkerStyle(ROOT.kOpenSquare)
        invMassHistoLS_copy.SetMarkerColor(ROOT.kGreen+2)
    ...
    leg1.AddEntry(invMassHistoLS_copy, "Like Sign", "pe")
