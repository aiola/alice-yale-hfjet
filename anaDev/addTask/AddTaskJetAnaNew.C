...
void AddTaskJetAnaNew(const char *cDataType = "AOD", const char *cRunType = "local",
                      const UInt_t kPhysSel = AliEmcalPhysicsSelection::kEmcalOk,
                      Bool_t bDoHF = kFALSE, Bool_t bDoChargedJets = kFALSE, Bool_t bDoFullJets = kFALSE,
                      Bool_t bDoTender = kTRUE, Bool_t bDoReclusterize = kTRUE, Bool_t bDoHadCorr = kTRUE, Bool_t bDoTrackingQA = kFALSE)
{
  ...
  TString sClusName("caloClusters");
  ...
  if (bDoFullJets) {
    ...
    AliAnalysisTaskSAJF *pSpectraFuTask = AddTaskSAJF(sTracksName, sClusName, sFuJetsName, "", kJetRadius, kJetPtCut, kJetAreaCut, "EMCAL"); 
