...
AliRDHFCutsD0toKpi* MakeD0toKpiCuts_PbPb2015_Cent30_50_Raa_QM17_XPeng(Bool_t saveFile = kTRUE)
{
  Float_t minc = 30;
  Float_t maxc = 50;
...
//  RDHFD0toKpi->SetMaxDifferenceTRKV0Centraltity(5.);
  ...
  if (saveFile) {
    TFile* fout = new TFile("RDHFCuts_Raa_D0_LHC15o_pass1_Cent2.root", "recreate");
    fout->cd();
    RDHFD0toKpi->Write();
    fout->Close();
  }
