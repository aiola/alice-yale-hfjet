...
void runEMCalJetAnalysisOld(const char   *cDataType      = "AOD",                                   // set the analysis type, AOD or ESD
			    ...
			    )
{
  ...
  const Bool_t   bDoTrackingQA        = kFALSE;
  ...
  if (bDoTender || bDoReclusterize) {
    ...
    if (1) {
      AliAnalysisTaskSAQA *pQATaskAfter = AddTaskSAQA("", sOrigClusName, sCellName, "", "",
						      0, 0, 0, 0., 0., "TPC", "AliAnalysisTaskSAQA_AfterTender");
    ...
    if (1) {
      AliAnalysisTaskSAQA *pQATaskAfterMaker = AddTaskSAQA("", sClusName, "", "", "",
							   0, 0, 0, 0., 0., "TPC", "AliAnalysisTaskSAQA_AfterClusterMaker");
  ...
  if (bDoHadCorr) {    
    ...
    if (bUseNewCode) {
      ...
      pMatcherTask->SetAttachEmcalParticles(kTRUE);
    }
    else {
  ...
  //pMgr->AddClassDebug("AliEmcalClusTrackMatcherTask", AliLog::kDebug);
